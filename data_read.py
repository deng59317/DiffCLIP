...
def load_data(dataset):
    if dataset != "Trento":
        raise ValueError("This data_read.py supports Trento patch dataset only.")
    ...
    if patch_HSI.shape[0] != y.shape[0]:
        raise ValueError(f"HSI N != y N: {patch_HSI.shape[0]} vs {y.shape[0]}")
    if patch_LiDAR.shape[0] != y.shape[0]:
        raise ValueError(f"LiDAR N != y N: {patch_LiDAR.shape[0]} vs {y.shape[0]}")
...
def readdata(type, dataset, windowsize, train_num, val_num, num):
    ...
    if type == "PCA":
        ...
        # 你 config 默认 none，这里只保留必要分支
        pass

    patch_HSI = _normalize01(patch_HSI)
    patch_LiDAR = _normalize01(patch_LiDAR)

    num_classes = int(y.max()) + 1 if y.size > 0 else 1
